...
const getAllUser = async () => {
        const result = await UserModel.find({}, { username: 1, fullName: 1, age: 1, email: 1, address: 1, _id: 0 }).select('-password')
        return result;
}
...
const getTotalPrice = async (userId: string) => {
        ...
        const totalPrice = total?.orders?.reduce((total, order) => total + order.price * order.quantity, 0)
